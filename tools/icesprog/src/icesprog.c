...
enum MCO_SOURCE_E
{
    ...
    MCO_HSE,     /* 12MHz */
    MCO_PLLCLK,
    /* 36MHz */ /* actually is RCC_CFGR_MCO_PLLCLK_DIV2 */
    MCO_SYSCLK, /* 72MHz */
    MCO_MAX,
};
...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        { /* read spi-flash */
            ...
            // ДОБАВИМ ПРОВЕРКУ ДОСТУПА К ФАЙЛУ
            fprintf(stdout, "Opening file for write...\n");
            ...
            ssize_t written = write(fd, flash_buf, len);
            if (written != len)
            {
                perror("write failed");
                fprintf(stderr, "Expected %d bytes, wrote %zd bytes\n", len, written);
                close(fd);
                exit(-1);
            }

            close(fd);
            fprintf(stdout, "File written successfully: %s\n", ifile);
        }
        else if (op_mode == 1)
        { /* write spi-flash */
            fprintf(stdout, "flash offset: 0x%08x\r\n", flash_offset);
