...
private:
    ...
    void tick() {
        ...
        if (++src_clk_counter >= SRC_CLK_RATIO) src_clk_counter = 0;
        if (++dst_clk_counter >= DST_CLK_RATIO) dst_clk_counter = 0;
        if (++pixel_valid_counter >= PIXEL_VALID_RATIO) pixel_valid_counter = 0;
        ...
        for (int x = 0; x < DST_WIDTH; x++) {
            ...
            if (pixel.valid) {
                ...
                if (src_y < SRC_HEIGHT && src_x < SRC_WIDTH) {
                    // FIXED: remove +1 from coordinates
                    uint32_t expected_color = ((frame_num & 0xF) << 20) | 
                                             ((src_y & 0xFFF) << 12) | 
                                             (src_x & 0xFFF);
...
public:
    ...
    uint32_t make_rgb(int frame_num, int x, int y) {
        return ((frame_num & 0xF) << 20) | (((y) & 0xFFF) << 12) | ((x) & 0xFFF);
    }
            ...
            while (cycles < 10000000 && !frame_done) {
                ...
                if (input_y < input_frame.getHeight()) {
                    ...
                    if (pixel_valid_counter == 0) {
                        ...
                        if (pixel.valid) input_pixels_sent++;
                    } else {
                ...
                if (dut->dst_clk_i) {
                    int out_x = dut->debug_dst_x_count_o-1;
                    int out_y = dut->debug_dst_y_count_o-1;
