...
**Важно:** Устройства в MMIO-пространстве **жёстко привязаны** к своим адресам (например, `PIC Controller` на `FC0020`, `DMA Controller` на `FC0200`). Адрес устройства является частью его архитектурного контракта.
...
**Альтернативный выход:** Аппаратура также может выключить `supervisor_mode` при попытке выборки команды по адресам `0000h` или `0038h`, если активен `supervisor_hook`.
...
- User Legacy Mode: При `mmio_userlock=1` запрещен доступ к портам D0, D3. Разрешен только D400 (SysCall) и все стандартные CPC-порты.
