...
`endif
  ...
  // =================================================
  // ECP5 TRELLIS_IO сериализация
  // =================================================

  reg [3:0] tmds_bit_counter = 0;
  reg tmds_load = 0;
  
  // Счетчик битов для сериализации
  always @(posedge i_tmdsclk) begin
    if (i_reset) begin
      tmds_bit_counter <= 0;
      tmds_load <= 0;
    end else begin
      tmds_load <= (tmds_bit_counter == 0);
      if (tmds_bit_counter == TMDS_WIDTH - 1) begin
        tmds_bit_counter <= 0;
      end else begin
        tmds_bit_counter <= tmds_bit_counter + 1;
      end
    end
  end

  // Регистры для данных TMDS
  ...
  // Мультиплексор выбора бита для сериализации
  wire red_bit, grn_bit, blu_bit;
  
  assign red_bit = tmds_shift_red[tmds_bit_counter];
  assign grn_bit = tmds_shift_grn[tmds_bit_counter];
  assign blu_bit = tmds_shift_blu[tmds_bit_counter];

  // TRELLIS_IO примитивы для каждого TMDS канала
  TRELLIS_IO #(
    .DIR("OUTPUT"),
    .ODDR_MODE("DDRX1"),  // Режим DDR
    .TSMUX("0"),          // Always output (not tristate)
    .SLEWRATE("FAST")     // Быстрые фронты
  ) trellis_red (
    .B(o_red),           // Выходной порт
    .I(red_bit),         // Данные для выхода
    .T(1'b0),            // Не tristate
    .O(),                // Вход не используется
    .OSCLK(i_tmdsclk),   // Быстрый clock для сериализации
