// =============================================================================
// HDMI Test System with Direct Scaler + TMDS Integration
// =============================================================================
...
module hdmi_kiss_test (
    ...
    // =========================================================================
    // 4. HDMI Scaler Core (ПРЯМОЕ ПОДКЛЮЧЕНИЕ)
    // =========================================================================
    ...
    hdmi_scaler_core #(
        ...
        .dst_de_o(scaler_de),                   // Data Enable
    ...
    // =========================================================================
    // 5. TMDS Encoding (ПРЯМОЕ ПОДКЛЮЧЕНИЕ)
    // =========================================================================
    ...
    // Control bits for blue channel (VSYNC, HSYNC)
    assign control = {scaler_vsync, scaler_hsync};

    // Red Channel TMDS Encoder
    tmds_encoder encoder_red (
        .clk_i(clk_27M),
        .rst_i(reset_active),
        .data_i(scaler_pixel_data[23:16]),  // Red channel
        .control_i(2'b00),
        .data_enable_i(scaler_pixel_stb),
        .tmds_o(tmds_red)
    );
    ...
    tmds_encoder encoder_green (
        .clk_i(clk_27M),
        .rst_i(reset_active),
        .data_i(scaler_pixel_data[15:8]),   // Green channel
        .control_i(2'b00),
        .data_enable_i(scaler_pixel_stb),
        .tmds_o(tmds_green)
    );

    // Blue Channel TMDS Encoder (carries sync signals)
    tmds_encoder encoder_blue (
        .clk_i(clk_27M),
        .rst_i(reset_active),
        .data_i(scaler_pixel_data[7:0]),    // Blue channel
        .control_i(control),                // HSYNC and VSYNC
        .data_enable_i(scaler_pixel_stb),
        .tmds_o(tmds_blue)
    );

    // =========================================================================
    // 6. TMDS Serialization
    // =========================================================================
    
    // Red Channel Serializer
    serializer #(.WIDTH(10)) ser_red (
        .clk_pixel(clk_27M),
        .clk_10x(clk_270M),
        .rst(reset_active),
    ...
    // TMDS Clock (direct connection)
    assign tmds_clock = clk_27M;

    // =========================================================================
    // 7. LED Indicators
    // =========================================================================
    assign led_g_o = gen_hsync;  // Input Hsync on green LED
    assign led_b_o = gen_vsync;  // Input Vsync on blue LED

    // =========================================================================
    // 8. DDR Outputs for TMDS Data Lines
    // =========================================================================
